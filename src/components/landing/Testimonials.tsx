    ...
    <section className="py-24 bg-[#151423]">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto">Join thousands of satisfied organizations that trust NameFrame.</p>
        </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                  ...
                  <div className="flex items-center mb-1">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i} 
                        size={14}
                        className={i < testimonials[currentTestimonial].rating ? "fill-yellow-400 text-yellow-400" : "text-gray-600"} 
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="md:w-3/4">
                  <blockquote className="text-gray-300 mb-4">"{testimonials[currentTestimonial].text}"</blockquote>
                  <div className="text-sm">
                    <div className="font-semibold text-white">{testimonials[currentTestimonial].name}</div>
                    <div className="text-violet-400">{testimonials[currentTestimonial].role}</div>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
              ...
              {testimonials.map((_, index) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full transition-all ${
                    currentTestimonial === index ? "bg-violet-500 w-6" : "bg-gray-600"
                  }`}
