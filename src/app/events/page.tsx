...
export default function EventsPage() {
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        const transformedEvents = data.data.map((event: any) => {
          ...
          return {
            ...
            description:
              event?.template.description || "No description available",
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="min-h-screen flex w-full bg-gradient-to-br pt-24 from-slate-900 via-gray-800 to-slate-900 text-white p-8 relative overflow-hidden"
    >
      ...
      <div className="max-w-7xl  mt-10 mx-auto relative">
        ...
        >
          <div>
            <h1 className="text-4xl text-center font-bold bg-gradient-to-r from-[#c7b9d3] via-[#ffffff] to-[#542082] text-transparent bg-clip-text">
              Your Events
            </h1>
        ...
        <div className="flex  gap-x-4 mb-8  ">
          <div className="relative flex gap-x-2  justify-center items-center flex-1">
            <input
              type="text"
              placeholder="Search events..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-2/3 pl-12 font-semibold pr-4 py-2 border-2  bg-gray-300 text-black backdrop-blur-sm  border-[#4b3a70]/30 rounded-lg focus:outline-none focus:border-[#b7a2c9] transition-all"
            />
        ...
        {/* Events Grid */}
        {loading ? (
          <div className="flex justify-center mt-20 items-center h-64">
            <DotLottieReact src="/hand.lottie" loop autoplay />
          </div>
        ) : (
            ...
            {filteredEvents.map((event) => (
              <Link key={event.id} href={`/events/${event.id}`}>
                <motion.div
                  ...
                  className="group relative bg-gradient-to-br from-[#0b1323] to-[#11161f] text-black backdrop-blur-xl rounded-lg p-6 border border-[#4b3a70]/30 shadow-lg overflow-hidden cursor-pointer"
                >
                  {/* Hover Effect */}
                  ...
                  <div className="relative">
                    <h3 className="text-xl font-bold text-gray-200 mb-2 group-hover:text-[#ffffff] transition-colors">
                      {event.name}
                    </h3>
                    <div className="border-b border-gray-600 border-1 " />
                    <p className="text-[#5c5b5b] mb-4 line-clamp-2">
                      {event.description}
                    </p>
